...
* Reduce cognitive load and easy transition from tidyverse to time series analysis.
* `%>%` chains individual verbs to compose a sound pipeline.
* Empower users to concentrate on data analysis, free from implementation.
...
.right-column[
```{r nyc-weekly-plot, fig.height = 9}
```
]
...
.right-column[
```{r nyc-monthly-plot, fig.height = 9}
```
]
